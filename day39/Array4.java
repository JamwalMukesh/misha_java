 ...
 */
 ...
 class MyArray
 {
     ...
     // Rotation happens in circular fashion 
     // [1,2,3,4]
     ...
     // Right Rotation by 1
     // [4,1,2,3]
     // It is a shifting alogrithm
     public void leftRotation(int times)
     {
        for(int count = 1; count <= times; count++)
        {
            // Single Rotation
            int element = array[0]; // store first element in to a variable
            // Iterate the array begin from 2nd position
            for(int i = 1; i < size; i++)
            {
                array[i-1] = array[i]; // shift the ith element to (i-1)th element
            }
            array[size-1] = element; // store origin first element of array at last position
        }
     }
 }
 class Array4
